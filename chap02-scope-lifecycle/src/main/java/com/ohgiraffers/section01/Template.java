...
public class Template {
    ...
    * 애플리케이션이 실행되는 동안 차례 SqlsessionFactory 를 다시 빌드하지 않는 것이 가장 좋은 형태이다
    ...
    * */
    ...
    public static SqlSession getSqlSession() {
        ...
        * 권장 사항이며 SqlSessionFactoryBuilder는 필요시에만 메소드 레벨에서 잠시 호출하는 것이 바람직하다

        * */

       if (sqlSessionFactory == null) { //sqlSessionFactory가 없다면 실행될 부분
       ...
       *  요청 시 생성하고 요청이 완료되면 close 하는 HTTP 요청과 스코프에 두는 것이 가장 올바른 방법이다.
       *
       * openSession() : SqlSession 인터페이스 타이브이 객체를 반환하는 메소드로 boolean 타입을 인자로 전달.
       * SqlSessionFactory 는 하나만 만들어 둔후 계속 사용하지만 그에 반환되는 SqlSession은
